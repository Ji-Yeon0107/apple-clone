(() => {
  ...
  let enterNewScene = false; // new scene에 진입한 순간 true가 된다
  ...
  function setLayout() {
    ...
    for (let i = 0; i < sceneInfo.length; i++) {
      if (sceneInfo[i].type === "sticky") {
        ...
        // 특별한 애니메이션이 없는 경우, 컨텐츠 길이만큼을 scrollHeight으로 정한다
        sceneInfo[i].scrollHeight = sceneInfo[i].objs.conatainer.offsetHeight;
      }
      sceneInfo[
        i
  ...
  function calcValues(values, currentYOffset) {
    // currentYOffset: 현재 scene에서 맨 처음을 0이라고 보는 YOffset의 값
    let rv;
    // currnet scene 에서 스크롤된 범위를 비율로 구하기
    ...
    if (values.length === 3) {
      ...
      const abc = (currentYOffset - partScrollStart) / partScrollHeight;
      if (
        currentYOffset >= partScrollStart &&
        currentYOffset <= partScrollEnd
      ) {
        rv = abc * (values[1] - values[0]) + values[0];
